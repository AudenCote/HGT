...
output_handle = "../Filtered_spreadsheets/gene_families_by_major_clade_no_duplicates_filtered_by_coverage.csv"
...
for rtg_file in os.listdir(rtg_files_handle):
	...
	try:
		...
		ogs_in_file = list(dict.fromkeys([record.id[-10:] for r, record in enumerate(list(SeqIO.parse(rtg_files_handle + '/' + rtg_file, "fasta"))) if covs_in_file[r] > 10]))
	except:
		continue
