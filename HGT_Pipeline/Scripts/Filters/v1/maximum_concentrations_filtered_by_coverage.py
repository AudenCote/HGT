...
numbers_output_handle = "../Master_spreadsheets/maximum_concentration_in_major_clade_filtered_by_coverage.csv"
names_output_handle = "../Master_spreadsheets/maximum_concentration_organisms_filtered_by_coverage.csv"
...
for f, rtg_file in enumerate(os.listdir(rtg_files_handle)):
	...
	try:
		...
		ogs_in_file = [record.id[-10:] for p, record in enumerate(list(SeqIO.parse(rtg_files_handle + '/' + rtg_file, "fasta"))) if covs_in_file[p] > 10]
	except:
		continue
